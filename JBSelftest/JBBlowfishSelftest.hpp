//------------------------------------------------------------------------------------------------------
// Blowfish selftest, (w) 2025 Jan Buchholz
//------------------------------------------------------------------------------------------------------
...
inline bool BlowfishSelfTest() {
    constexpr uint32_t PLAINH[] = {
    ...
    constexpr int loop_ck = 34;
    constexpr int loop_vk = 24;
    ...
    for (i = 0; i < loop_ck; i++) {
        ...
        key.resize(8);
        for (j = 0; j < 8; j++) key[j] = KEYV[k + j];
        k += 8;
        zl = PLAINH[i];
        zr = PLAINL[i];
        BlowfishInitialize(key);
        BlowfishEncryptBase(&zl, &zr);
        if (zl != CIPHH[i] || zr != CIPHL[i]) return false;
        BlowfishDecryptBase(&zl, &zr);
        if (zl != PLAINH[i] || zr != PLAINL[i]) return false;
    }
    /* variable key size */
    for (i = 0; i < loop_vk; i++) {
        uint8_t const KEYS[] = {
        ...
        key.resize(i + 1);
        for (j = 0; j <= i; j++) key[j] = KEYS[j];
        zl = PLAINH[loop_ck + i];
        zr = PLAINL[loop_ck + i];
        BlowfishInitialize(key);
        BlowfishEncryptBase(&zl, &zr);
        if (zl != CIPHH[loop_ck + i] || zr != CIPHL[loop_ck + i]) return false;
        BlowfishDecryptBase(&zl, &zr);
        if (zl != PLAINH[loop_ck + i] || zr != PLAINL[loop_ck + i]) return false;
    }
    return true;
}

inline void JBBlowfishSelftest() {
    const std::string res = (BlowfishSelfTest() ? "PASSED" : "FAILED");
    const std::string key = "Batteries not included";
    const std::vector<uint8_t> ivec = { 0x01, 0xaa, 0xff, 0x55, 0x23, 0x91, 0x45, 0xee };
    std::cout << "\n---------------- Blowfish Selftest ----------------" << std::endl;
    std::cout << "... using standard test vectors: " << res << std::endl;
    std::cout << "\n---------------- Blowfish ECB Test ----------------" << std::endl;
    ...
    std::vector<uint8_t> enc = BlowfishEncryptECB(plain);
    std::cout << "Encrypted text: " << vectorToHexString(enc) << std::endl;
    std::vector<uint8_t> dec = BlowfishDecryptECB(enc);
    std::cout << "Decrypted text: " << std::string(dec.begin(), dec.end()) << std::endl;
    std::cout << "\n---------------- Blowfish CBC Test ----------------" << std::endl;
    std::cout << "Encryption key  : " << key << std::endl;
    std::cout << "Init. CBC vector: " << vectorToHexString(ivec) << std::endl;
    std::cout << "Plain text      : " << text1 << std::endl;
    enc = BlowfishEncryptCBC(plain, ivec);
    std::cout << "Encrypted text  : " << vectorToHexString(enc) << std::endl;
    dec = BlowfishDecryptCBC(enc, ivec);
    std::cout << "Decrypted text  : " << std::string(dec.begin(), dec.end()) << std::endl;
}
